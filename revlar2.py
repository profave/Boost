...
def overview():
    print(f"\033[1;91m  ━━━━━━━━━━━━━━━━━[{red}𝙔𝙊𝙐𝙍 𝘼𝘾𝘾𝙊𝙐𝙉𝙏𝙎{red}]━━━━━━━━━━━━━━━━━━━")
    total_accounts = count_lines("/sdcard/Aqua/toka.txt")
    total_pages = count_lines("/sdcard/Aqua/tokp.txt")
    print(f"  {r}     TOTAL ACCOUNTS: {dark_gray}{total_accounts}{r}")
    print(f"  {r}     TOTAL PAGES: {dark_gray}{total_pages}")
    print(f'{red}  ══════════════════════════════════════════════════{wh}')
...
def greet_user():
    ...
    if current_time.hour < 12:
        print(f"    Good morning mother fucker, {red}{user_name}!{dark_gray}")
    elif current_time.hour < 18:
        print(f"    Good afternoon mother fucker, {red}{user_name}!{dark_gray}")
    else:
        print(f"    Good evening mother fucker, {red}{user_name}!{dark_gray}")
...
def ip():
    ...
    print(f" {red}  「{dark_gray}State{r}」{w}:{red}THIS IS 50% FREE")
    print(f" {red}  「{dark_gray}IP{r}」{w}:{red}{ip_response.get('query', ' ')}")
    print(f"{red}   「{dark_gray}country{r}」{w}:{red}{ip_response.get('country', ' ')}")
    print(f"{red}   「{dark_gray}ISP{r}」{w}:{red}{ip_response.get('isp', ' ')}")
    print(f"{red}   「{dark_gray}s-key{r}」{w}:{red}SUCK MY DICK {red}")
    linex()
